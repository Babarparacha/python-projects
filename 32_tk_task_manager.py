...
def mark_completed():
    try:
        ...
        if task.startswith("✓"):
            task = task[2:]  # Remove checkmark
        else:
            task = "✓ " + task  # Add checkmark
        task_listbox.delete(selected_task_index)
...
task_entry = tk.Entry(input_frame, width=30, font=("Arial", 10))
...
task_listbox = tk.Listbox(list_frame, width=45, height=12, font=("Arial", 12), selectmode=tk.SINGLE, yscrollcommand=scrollbar.set)
